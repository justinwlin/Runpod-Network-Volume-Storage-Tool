...
from runpod_storage import AuthenticationError, RunpodStorageAPI, VolumeNotFoundError
...
def main():
    ...
    try:
        ...
        for volume in volumes:
            print(
                f"   • {volume['name']} ({volume['id']}) - {volume['size']}GB in {volume['dataCenterId']}"
            )
    ...
    if not os.getenv("RUNPOD_S3_ACCESS_KEY") or not os.getenv("RUNPOD_S3_SECRET_KEY"):
        ...
        print(
            "   To enable file operations, set RUNPOD_S3_ACCESS_KEY and RUNPOD_S3_SECRET_KEY"
        )
    else:
