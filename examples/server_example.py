...
class RunpodStorageClient:
    ...
    def upload_file(self, volume_id: str, file_path: str, remote_path: str = None) -> Dict[str, Any]:
...
def main():
    ...
    if volumes:
        ...
        if not os.getenv("RUNPOD_S3_ACCESS_KEY") or not os.getenv("RUNPOD_S3_SECRET_KEY"):
            ...
            print("   Set RUNPOD_S3_ACCESS_KEY and RUNPOD_S3_SECRET_KEY to test file operations")
        
        try:
            # Create test file
            with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as f:
                f.write("Hello from API server example!\nTimestamp: " + str(os.time.time()))
                test_file = f.name
            
            # Upload file
            print("   📤 Uploading test file...")
            upload_result = client.upload_file(volume_id, test_file, "api-demo/test.txt")
            print(f"   ✅ Upload completed: {upload_result['file_path']} ({upload_result['size']} bytes)")
        ...
        print("     -d '{\"name\": \"test-volume\", \"size\": 10, \"datacenter_id\": \"EU-RO-1\"}'")
