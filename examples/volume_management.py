...
def update_volume_example(volume_id):
    ...
    try:
        ...
        updated_volume = api.update_volume(
            volume_id, 
            name="renamed-example-volume"
        )
        print(f"✅ Renamed to: {updated_volume['name']}")
        
        # Then, expand the size
        print("📈 Expanding volume size...")
        expanded_volume = api.update_volume(
            volume_id, 
            size=50  # Expand to 50GB
        )
        ...
        final_volume = api.update_volume(
            volume_id,
            name="large-example-volume",
            size=100  # Expand to 100GB
        )
