...
class RunpodClient:
    ...
    def list_network_volumes(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return (
                ...
                else response.get("networkVolumes", [])
            )
    ...
    def create_network_volume(
        ...
        if datacenter_id not in self.DATACENTERS:
            raise ValueError(
                f"Invalid datacenter. Must be one of: {list(self.DATACENTERS.keys())}"
            )

        if not (10 <= size <= 4000):
            raise ValueError("Size must be between 10 and 4000 GB")

        data = {"name": name, "size": size, "dataCenterId": datacenter_id}
    ...
    def update_network_volume(
        self, volume_id: str, name: Optional[str] = None, size: Optional[int] = None
    ) -> Dict[str, Any]:
